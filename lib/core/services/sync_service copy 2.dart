  ...
  Future<bool?> savesyncfirst(context, conntype) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final planterjson = Planterjson.fromJson(jsonData);
        final plantersourcejson = PlanterSourcejson.fromJson(jsonData);
        final lotcodejson = LotCodejson.fromJson(jsonData);
        final requestsjson = Requestjson.fromJson(jsonData);
        final coordinatorjson = Coordinatorjson.fromJson(jsonData);
        final cuttersjson = Cutterjson.fromJson(jsonData);
        final varietyjson = Varietyjson.fromJson(jsonData);
        final locationsjson = SrcLocationjson.fromJson(jsonData);
        final cmjson = CMjson.fromJson(jsonData);
        final permissionsJson = PermissionsJson.fromJson(jsonData);
        final cprjson = Cprjson.fromJson(jsonData);
        final lotpicturesjson = LotPictureJson.fromJson(jsonData);
        ...
        await db.transaction((txn) async {
          ...
          for (final item in planterjson.data) {
            final plid = int.tryParse(item.plid ?? '0') ?? 0;
            final plcode = item.plcode ?? '';
            final plname = item.plname ?? '';
            final traflag = item.traflag ?? '';
          ...
          for (final item in plantersourcejson.data) {
            final plsourceId = int.tryParse(item.plsource_id ?? '0') ?? 0;
            final plsourceCode = item.plsource_code ?? '';
            final plsourceName = item.plsource_name ?? '';
            final traflag = item.traflag ?? '';
            var x = SourcePlanter(
              plsourceId,
              plsourceCode,
          ...
          for (final item in lotcodejson.data) {
            final lotcodeid = int.tryParse(item.lotcodeid ?? '0') ?? 0;
            final lotcodename = item.lotcodename ?? '';
            final plantersrcId = int.tryParse(item.plantersrcId ?? '0') ?? 0;
            final traflag = item.traflag ?? '';
          ...
          for (final item in requestsjson.data) {
            final requestId = int.tryParse(item.request_id ?? '0') ?? 0;
            final requestNo = int.tryParse(item.request_no ?? '0') ?? 0;
            final requestDate = item.request_date ?? '';
            final plCode = item.pl_code ?? '';
            final plName = item.pl_name ?? '';
            final location = item.location ?? '';
            final area = item.area ?? '';
            final qty = (double.tryParse(item.qty ?? '0') ?? 0.0).toInt();
            final remainingQty =
                (double.tryParse(item.remaining_qty ?? '0') ?? 0.0).toInt();
            final deliveredQty =
                (double.tryParse(item.delivered_qty ?? '0') ?? 0.0).toInt();
            final plid = int.tryParse(item.plid ?? '0') ?? 0;
            final forcpr = int.tryParse(item.forcpr ?? '0') ?? 0;
            final traflag = item.traflag ?? '';
            var x = Requests(
              requestId,
              requestNo,
          ...
          for (final item in coordinatorjson.data) {
            final id = int.tryParse(item.fr_id ?? '0') ?? 0;
            final frCode = item.fr_code ?? '';
            final frName = item.fr_name ?? '';
            final traflag = item.traflag ?? '';
          ...
          for (final item in cuttersjson.data) {
            final id = int.tryParse(item.id ?? '0') ?? 0;
            final description = item.description ?? '';
            final traflag = item.traflag ?? '';

            final x = Cutters(id, description, traflag);
          ...
          for (final item in varietyjson.data) {
            final id = int.tryParse(item.id ?? '0') ?? 0;
            final description = item.description ?? '';
            final traflag = item.traflag ?? '';
          ...
          for (final item in locationsjson.data) {
            final id = int.tryParse(item.id ?? '0') ?? 0;
            final location = item.location ?? '';
          ...
          for (final item in cmjson.data) {
            final id = int.tryParse(item.id ?? '0') ?? 0;
            String desc = item.description ?? '';
            String traflag = item.traflag ?? '';
          ...
          for (final item in permissionsJson.data) {
            int? moduleid = int.tryParse(item.moduleid ?? '0') ?? 0;
            int? hasaccess = int.tryParse(item.hasaccess ?? '0') ?? 0;
          ...
          for (final item in cprjson.data) {
            final id = int.tryParse(item.id ?? '0') ?? 0;
            final cprId = int.tryParse(item.id ?? '0') ?? 0;
            final cprRefno = item.cpr_refno ?? '';
            final requestId = int.tryParse(item.request_id ?? '0') ?? 0;
            final locationId = int.tryParse(item.location_id ?? '0') ?? 0;
            final varietyId = int.tryParse(item.variety_id ?? '0') ?? 0;
            final planterId = int.tryParse(item.planter_id ?? '0') ?? 0;
            final cutterId = int.tryParse(item.cutter_id ?? '0') ?? 0;
            final qty = int.tryParse(item.qty ?? '0') ?? 0;
            final deliveryDate = item.delivery_date ?? '';
            final deliveredById =
                int.tryParse(item.delivered_by_id ?? '0') ?? 0;
            final recievedBy = item.recieved_by ?? '';
            final series = int.tryParse(item.series ?? '0') ?? 0;
            final sourcePlanter = int.tryParse(item.source_planter ?? '0') ?? 0;
            final traflag = item.traflag ?? '';
            final printCount = int.tryParse(item.print_count ?? '0') ?? 0;
            final rcvfrId = int.tryParse(item.rcvfr_id ?? '0') ?? 0;
            final haulingPaid = (double.tryParse(item.hauling_paid ?? '0') ?? 0)
                .toInt();

            final haulingAmount =
                (double.tryParse(item.hauling_amount ?? '0') ?? 0.0).toInt();

            final cuttingmode = int.tryParse(item.cuttingmode ?? '0') ?? 0;

            final cuttingdate = item.cuttingdate ?? '';

            final cuttingPaid =
                (double.tryParse(item.cutting_paid ?? '0') ?? 0.0).toInt();

            final cuttingAmount =
                (double.tryParse(item.cutting_amount ?? '0') ?? 0.0).toInt();

            final sacksPaid = (double.tryParse(item.sacks_paid ?? '0') ?? 0.0)
                .toInt();

            final sacksAmount =
                (double.tryParse(item.sacks_amount ?? '0') ?? 0.0).toInt();

            final othersPaid = (double.tryParse(item.others_paid ?? '0') ?? 0.0)
                .toInt();

            final othersAmount =
                (double.tryParse(item.others_amount ?? '0') ?? 0.0).toInt();
            final lotCode = item.lot_code ?? '';
          ...
          for (final item in lotpicturesjson.data) {
            final collpid = int.tryParse(item.collpid ?? '') ?? 0;
            final collpreqid = item.collpreqid ?? '';
            final collandprep = item.collandprep ?? '';
            final collandprepdate = item.collandprepdate ?? '';
            final collandactualdate = item.collandactualdate ?? '';
            final collandactual = item.collandactual ?? '';
            final collptraflag = item.collptraflag ?? '';
            final colaptraflag = item.colaptraflag ?? '';
        ...
        for (var item in lotpicturesjson.data) {
          await download_lp_Image(
            int.parse(item.collpid ?? '0') ?? 0,
            item.collpreqid ?? '',
          );
        }
